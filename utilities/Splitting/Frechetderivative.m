...
function [Frechet] = Frechetderivative(x1,x2,u,param)
Frechet=zeros(param.Nwaves*param.m,2*param.m+1);
...
for it=1:param.Nwaves
    ...
    Frechet(((it-1)*param.m+1):(it*param.m),1:param.m)=diag(W_pos);
    Frechet(((it-1)*param.m+1):(it*param.m),param.m+1:2*param.m)=diag(W_neg);
    Frechet(((it-1)*param.m+1):(it*param.m),2*param.m+1)=(x1.*a_pos/((u*param.effectivedT)^2)).*W_pos+(x2.*a_neg/((u*param.effectivedT)^2)).*W_neg;
end
end